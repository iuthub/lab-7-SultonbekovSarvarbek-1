...
-- #7
SELECT first_name FROM directors WHERE id IN (SELECT director_id FROM movies_directors WHERE movie_id IN (SELECT movie_id FROM movies_genres WHERE genre = "Horror"));

-- #8
SELECT first_name FROM actors WHERE id IN (SELECT actor_id FROM roles WHERE movie_id IN (SELECT movie_id FROM movies_directors WHERE director_id = (SELECT id FROM directors WHERE first_name = "Christopher" AND last_name = "Nolan")));
